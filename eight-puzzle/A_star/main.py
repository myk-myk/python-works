...
def heuristic_function(curr_state, finish_state):
    curr_state_list, finish_state_list = make_list(curr_state), make_list(finish_state)
    d, n = 0, 0
    for num, val in enumerate(curr_state_list):
        for curr_row in curr_state:
            if curr_row.__contains__(val):
                x_i = curr_row.index(val)
                y_i = curr_state.index(curr_row)
        for fin_row in finish_state:
            if fin_row.__contains__(val):
                x_j = fin_row.index(val)
                y_j = finish_state.index(fin_row)
        if val != 0:
            diff = abs(x_i - x_j) + abs(y_i - y_j)
            d += diff
        if curr_state_list[num] != finish_state_list[num]:
            n += 1
    h = d + n
    return h

def eight_game_Astar(curr_state, finish_state):
    depth, state_num = 1, 1
    visited, v_queue = {}, PriorityQueue()
    state_dict = {}
        ...
        while curr_state != finish_state:
            if curr_state in list(visited.values()):
                ...
                if curr_state not in list(visited.values()):
                    ...
                    func = heuristic_function(curr_state, finish_state)
                    file.write(f"{state_num}: h = {func}; {curr_state}\n")
                state_num += 1
                for gen_state in state_generator(curr_state, finish_state):
                    ...
                    if depth > max_depth:
                        ...
                        if neighbour_gen_state not in list(visited.values()):
                            func = heuristic_function(neighbour_gen_state, finish_state)
                            v_queue.put((func, neighbour_gen_state))
                        else:
                            v_queue.put((-1, neighbour_gen_state))
            ...
            if control_state == finish_state:
                ...
                if control_state not in list(visited.values()):
                    ...
                    func = heuristic_function(control_state, finish_state)
                    file.write(f"{state_num}: h = {func}; {control_state}\n")
                ...
                break
            elif not v_queue.empty():
                curr_state = v_queue.get()[1]
            else:
                if depth > max_depth:
...
if __name__ == '__main__':
    ...
    all_states, base_states = eight_game_Astar(start, finish)
